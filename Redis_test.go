...
func TestBase(t *testing.T) {
	...
	if !expireFlag {
		t.Fatal("Expire Bool", expireFlag)
	}
	time.Sleep(time.Duration(2) * time.Second)
	r = redis.GET("redisName")
	if r.Int() > 0 {
		t.Fatal("Expired err", r)
	}
	...
	if !expireFlag {
		t.Fatal("ExpireAt Bool", expireFlag)
	}
	r = redis.GET("redisName")
	if r.Int() > 0 {
		t.Fatal("ExpireAt err", r)
	}
	...
	if r.Error != nil && ru.Name != "aaa" || ru.Id != 123 || !ru.Time.Equal(info.Time) {
		t.Fatal("userInfo Struct", ru)
	}

	rm := map[string]interface{}{}
	r.To(&rm)
	if rm["name"] != "aaa" || rm["id"].(float64) != 123 {
		t.Fatal("userInfo Map", rm)
	}
	...
	if len(results) != 2 || results[0].String() != "Sam Lee" {
		t.Fatal("MGET Results", results)
	}
	ru2 := new(userInfo)
	results[1].To(ru2)
	if ru2.Name != "BBB" {
		t.Fatal("MGET Struct", results)
	}
...
func TestHash(t *testing.T) {
	...
	if len(results) != 2 || results[0].String() != "Sam Lee" {
		t.Fatal("HMGET", results[0])
	}
	ru2 := new(userInfo)
	results[1].To(ru2)
	if ru2.Name != "BBB" {
		t.Fatal("HMGET", results[1])
	}
